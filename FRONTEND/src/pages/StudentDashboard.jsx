          ...
          <div className="max-w-xl">
            ...
            <div className="rounded-2xl border border-slate-200 bg-white p-6">
              ...
              <div className="mt-6 border-t border-slate-100 pt-6">
                <p className="text-sm font-semibold text-slate-700 mb-3">Resume</p>
                {resumeFile ? (
                  <div className="flex items-center justify-between rounded-xl border border-slate-200 bg-slate-50 px-4 py-3">
                    ...
                    <button type="button" onClick={() => setResumeFile(null)} className="ml-2 text-xs font-semibold text-red-500 hover:underline shrink-0">Remove</button>
                  </div>
                ) : (
                  <div className="rounded-xl border-2 border-dashed border-slate-200 bg-slate-50 p-6 text-center">
                    <Upload className="mx-auto h-6 w-6 text-slate-400" />
                    <p className="mt-2 text-sm font-semibold text-blue-700">Select your resume file</p>
                    <p className="text-xs text-slate-400">PDF, DOC, or DOCX formats</p>
                  </div>
                )}
                <div className="mt-3 flex items-center gap-2">
                  <input type="file" className="block w-full text-sm text-slate-500 file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-sm file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100" accept=".pdf,.doc,.docx" onChange={(e) => setResumeFile(e.target.files[0])} />
                </div>
              </div>
            </div>
            ...
            <div className="mt-4 grid grid-cols-3 gap-3">
              {[
                { label: "Applied", value: applications.length, icon: Briefcase },
                { label: "Shortlisted", value: applications.filter(a => a.status === "Shortlisted").length, icon: BookOpen },
                { label: "Selected", value: applications.filter(a => a.status === "Selected").length, icon: Check },
              ].map(({ label, value, icon: Icon }) => (
                <div key={label} className="flex flex-col items-center rounded-2xl border border-slate-200 bg-white py-5 text-center">
