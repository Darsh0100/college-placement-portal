...
// ─── Job Card (WITH ELIGIBILITY CHECKS) ──────────────────────────────────────
    ...
    <div className="flex flex-col rounded-2xl border border-slate-200 bg-white p-6 transition hover:shadow-md">
      <div className="flex items-start justify-between gap-2">
        <div>
          ...
          <p className="text-sm font-medium text-blue-700">{displayCompanyName}</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <div>
            <p className="text-sm text-slate-500">
              You are applying for <span className="font-semibold text-slate-900">{job.title}</span> at{" "}
              <span className="font-semibold text-blue-700">{displayCompanyName}</span>.
            </p>
          </div>

          <div>
            <label className="block text-xs font-semibold text-slate-600 uppercase tracking-wider mb-1.5">
              Upload Resume (PDF Required) <span className="text-red-500">*</span>
            </label>
            {modalResumeFile ? (
              <div className="flex items-center justify-between rounded-xl border border-green-200 bg-green-50 px-4 py-3 text-sm text-green-800">
                <div className="flex items-center gap-2 truncate">
                  ...
                  <span className="font-medium truncate">{modalResumeFile.name}</span>
                </div>
                <button type="button" onClick={() => setModalResumeFile(null)} className="ml-2 text-xs font-bold text-red-500 hover:underline shrink-0">Change</button>
              </div>
            ) : (
              <div className="relative flex flex-col items-center justify-center rounded-xl border-2 border-dashed border-slate-200 bg-slate-50 p-4 text-center transition hover:border-blue-400">
                ...
                <p className="mt-1 text-xs font-medium text-blue-700">Click to select file</p>
                <input type="file" required accept=".pdf" onChange={(e) => setModalResumeFile(e.target.files[0])} className="absolute inset-0 cursor-pointer opacity-0" />
              </div>
            )}
          </div>

          <div>
            <label className="block text-xs font-semibold text-slate-600 uppercase tracking-wider mb-1">Short Note / Message (Optional)</label>
            <textarea rows={3} value={coverLetter} onChange={(e) => setCoverLetter(e.target.value)} placeholder="Briefly pitch yourself or list relevant technical focus skills..." className="w-full rounded-xl border border-slate-200 p-3 text-sm text-slate-900 placeholder-slate-400 outline-none transition focus:border-blue-600 focus:ring-2 focus:ring-blue-100" />
          </div>
...
function JobDetailModal({ job, applied, userBranch, onClose, onApply }) {
  ...
  const recruiterId = job.recruiter || (hasCompanyObj ? job.company.userId : "N/A");
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-blue-950/40 p-4 backdrop-blur-sm">
      <div className="w-full max-w-lg rounded-2xl bg-white shadow-2xl overflow-hidden">
        ...
        <div className="p-6 space-y-5">
          <div>
            <p className="text-xl font-bold text-blue-900">{displayCompanyName}</p>
            {industrySector && typeof industrySector === 'string' && (
              <p className="text-xs font-semibold text-slate-400 mt-0.5">{industrySector} Industry</p>
            )}
            <div className="mt-3 flex flex-wrap gap-x-4 gap-y-1.5 text-xs font-medium text-slate-500">
              ...
              <span className="flex items-center gap-1"><IndianRupee className="h-3.5 w-3.5 text-slate-400" />{job.salary}</span>
          ...
          <div>
            <p className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">Role Description</p>
            <p className="text-sm leading-relaxed text-slate-600 bg-slate-50/50 p-3 rounded-xl border border-slate-100">{job.description}</p>
          </div>
          <div className="grid grid-cols-2 gap-4 rounded-xl border border-slate-100 bg-slate-50/30 p-3.5 text-xs">
            <div>
              <p className="font-bold text-slate-400 uppercase tracking-wider mb-0.5">Corporate Email</p>
              <p className="font-medium text-slate-700 truncate">{typeof companyEmail === 'string' ? companyEmail : "Not Provided"}</p>
            </div>
            <div>
              <p className="font-bold text-slate-400 uppercase tracking-wider mb-0.5">Recruiter Reference ID</p>
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-1.5">
              {Array.isArray(job.skills) && job.skills.length > 0 ? (
                job.skills.map((s) => (
                  <span key={s} className="rounded-full bg-blue-50 px-2.5 py-0.5 text-xs font-semibold text-blue-700 border border-blue-100">{s}</span>
                ))
              ) : (
                <span className="text-xs text-slate-400">No specific technical benchmarks listed.</span>
              )}
            </div>
          </div>
          <div className="pt-2">
...
export default function StudentDashboard() {
  ...
  // Persist resume metadata state from localStorage on mount
  ...
  // Handle local resume file loading and storage string conversion
  const handleResumeUpload = (e) => {
    const file = e.target.files[0];
    if (!file) return;

    // Guard constraint check for storage capacity
    if (file.size > 2 * 1024 * 1024) {
      showToast("File size too large. Please upload a PDF under 2MB for local synchronization.", "error");
      return;
    }
    ...
    reader.onload = () => {
      const resumeMetadata = {
        name: file.name,
        type: file.type,
        base64: reader.result // File data payload string
      };
      
      localStorage.setItem("student_resume", JSON.stringify(resumeMetadata));
      setResumeFile(resumeMetadata);
      showToast("Resume saved securely to profile baseline dashboard sync parameters.", "success");
    };
  ...
  const handleResumeRemove = () => {
    ...
    setResumeFile(null);
    showToast("Resume template parameter cleared from profile configurations.", "success");
  };
  ...
  function handleLogout() {
    ...
    // NOTE: Keeping the local storage parameter token active so logout doesn't erase user profile documentation metadata.
    navigate("/login");
  }
  ...
  return (
    <div className="min-h-screen bg-slate-50 relative">
      <header className="sticky top-0 z-10 border-b border-slate-200 bg-white">
        ...
        <div className="mx-auto flex max-w-6xl gap-1 overflow-x-auto px-6">
          {tabs.map(({ key, label, icon: Icon }) => (
            <button key={key} type="button" onClick={() => setActiveTab(key)} className={`flex items-center gap-1.5 whitespace-nowrap border-b-2 px-4 py-3 text-sm font-semibold transition ${activeTab === key ? "border-blue-900 text-blue-900" : "border-transparent text-slate-400 hover:text-slate-700"}`}>
          ...
          <div>
            <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              ...
              <div className="relative w-full sm:w-72">
                ...
                <input className="w-full rounded-xl border border-slate-200 bg-white py-2.5 pl-10 pr-4 text-sm placeholder-slate-400 outline-none focus:border-blue-600 focus:ring-2 focus:ring-blue-100" placeholder="Search roles or companies..." value={search} onChange={(e) => setSearch(e.target.value)} />
              </div>
            </div>
          ...
          <div>
            ...
            {applications.length === 0 ? (
              <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-slate-200 bg-white py-16 text-center">
                ...
                <p className="text-sm text-slate-500">Apply to a job to track it here.</p>
              </div>
            ) : (
              <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white">
                <table className="w-full text-left text-sm">
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {applications.map((a) => {
                      const jobTitle = a.job?.title || a.jobTitle || "Unknown Position";
                      let companyName = "Unknown Company";
                      if (typeof a.company === 'object' && a.company !== null) {
                        companyName = a.company.CompanyName || a.company.name || companyName;
                      } else if (typeof a.job?.company === 'object' && a.job.company !== null) {
                        companyName = a.job.company.CompanyName || a.job.company.name || companyName;
                      ...
                      return (
                        <tr key={a._id || a.jobId}>
                          <td className="px-5 py-4 font-medium text-slate-900">{jobTitle}</td>
                          <td className="px-5 py-4 text-slate-500">{companyName}</td>
                          <td className="px-5 py-4 text-slate-500">{displayDate}</td>
                          <td className="px-5 py-4">
                            <span className={`inline-flex items-center rounded-full border px-2.5 py-1 text-xs font-medium ${STATUS_STYLE[a.status] || STATUS_STYLE.Applied}`}>
                              {a.status || "Applied"}
                            </span>
                          </td>
                        </tr>
            ...
            <div className="rounded-2xl border border-slate-200 bg-white p-6">
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1">
                  ...
                  {/* Read-Only Display for Branch Allocation Setup */}
                  <div className="mt-3">
                    <span className="block text-[11px] font-bold text-slate-400 uppercase tracking-wide">Assigned Academic Branch</span>
                    <p className="mt-1 inline-block rounded-xl border border-slate-200 bg-slate-50 px-3 py-1.5 text-xs font-bold text-slate-700">
                      {userBranch || "Unspecified"}
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="mt-6 border-t border-slate-100 pt-6">
                <p className="text-sm font-semibold text-slate-700 mb-3">Resume Document</p>
                {resumeFile ? (
                  <div className="flex items-center justify-between rounded-xl border border-slate-200 bg-slate-50 px-4 py-3">
                    <div className="flex items-center gap-2 text-sm text-slate-700 min-w-0 flex-1">
                      <FileText className="h-4 w-4 text-blue-700 shrink-0" />
                      <span className="truncate pr-2 font-medium">{resumeFile.name}</span>
                    </div>
                    <button type="button" onClick={handleResumeRemove} className="ml-2 text-xs font-semibold text-red-500 hover:underline shrink-0">Remove</button>
                  </div>
                ) : (
                  <div className="rounded-xl border-2 border-dashed border-slate-200 bg-slate-50 p-6 text-center">
                    <Upload className="mx-auto h-6 w-6 text-slate-400" />
                    <p className="mt-2 text-sm font-semibold text-blue-700">Select your resume file</p>
                    <p className="text-xs text-slate-400">PDF formats (Max 2MB)</p>
                  </div>
                )}
                <div className="mt-3 flex items-center gap-2">
                  <input type="file" className="block w-full text-sm text-slate-500 file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-sm file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100 cursor-pointer" accept=".pdf" onChange={handleResumeUpload} />
                </div>
              </div>
            </div>
          </div>
      ...
      {/* Floating Notifications UI Layer */}
      {toast && (
        <div className="fixed bottom-24 right-5 z-50 flex max-w-sm items-start gap-3 rounded-2xl border bg-white p-4 shadow-xl border-slate-100 transition-all duration-300 animate-in fade-in slide-in-from-bottom-4">
          {toast.type === "error" ? (
          ...
          <button type="button" onClick={() => setToast(null)} className="rounded-lg p-1 text-slate-400 hover:bg-slate-50 transition">
            <X className="h-4 w-4" />
          </button>
        </div>
      )}

      {/* Static ChatBot Fixed Wrapper Layer */}
